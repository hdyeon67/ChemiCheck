// 계측 헬퍼 (analytics-spec.md 준수, app="chemicheck")
//   메인: PostHog(쿠키리스 memory persistence) · 보조: Cloudflare Web Analytics
//   원칙: 개인정보 미저장 — 이벤트 속성에 이름·생년월일·입력값 절대 포함 금지.
//         관계(category)·점수 구간(score_band) 등 비식별 값만 허용.
//
...
/** chemicheck 이벤트 (analytics-spec.md 공통 스키마) */
export type ChemiEvent =
  | "landing_view"
  | "input_submit"
  | "result_view"
  | "share_click"
...
export function track(event: ChemiEvent, props: Record<string, unknown> = {}): void {
  if (typeof window === "undefined") return;
  try {
    window.posthog?.capture(event, { app: "chemicheck", ...props });
  } catch {
    /* 계측 실패는 조용히 무시 */
  }
}
